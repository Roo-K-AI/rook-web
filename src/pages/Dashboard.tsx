...
export function Dashboard() {
  ...
  const stats = {
    ...
    today: products.filter((p) => p.created_at && new Date(p.created_at).toDateString() === today).length,
    completed: products.filter((p) => p.rook_status === 'completed').length,
    pending: products.filter((p) => p.rook_status === 'accepted' || p.rook_status === 'processing').length,
  };
      ...
      <div className="px-6 sm:px-8 pb-12 space-y-8">
            ...
            return (
              ...
              >
                <Card hover className="p-5">
                  ...
                  <p className="text-2xl font-bold text-ink dark:text-ink-dark">{s.value}</p>
                  <p className="text-sm text-ink-muted dark:text-ink-dark-muted mt-0.5">{s.label}</p>
                </Card>
              </motion.div>
            );
          })}
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-bold text-ink dark:text-ink-dark">Recent Generations</h2>
          ...
          {loading ? (
            ...
            <Card className="p-12 text-center">
              ...
              <h3 className="text-base font-bold text-ink dark:text-ink-dark">No products yet</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {products.slice(0, 6).map((p, i) => (
                <motion.div
                  ...
                  transition={{ delay: i * 0.05 }}
                >
                  <Card hover className="p-5" onClick={() => navigate(`/product/${p.id}`)}>
                    ...
                    <h3 className="font-semibold text-ink dark:text-ink-dark truncate">{p.name}</h3>
