...
function SectionCard({
  ...
  const copyText = isList
    ? (content as string[]).map((c) => `• ${c}`).join('\n')
    : (content as string);
    ...
    >
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-sm font-bold text-ink dark:text-ink-dark">{section.label}</h3>
        </div>
        <CopyButton text={copyText} label={section.label} />
      </div>

      {isList ? (
        <ul className="space-y-2">
          {(content as string[]).map((item, i) => (
            <li key={i} className="flex items-start gap-2.5 text-sm text-ink dark:text-ink-dark">
...
export function ProductSheet({ product, onNew }: ProductSheetProps) {
  const { show } = useToast();

  const sections: Section[] = [
    { key: 'seo_title', label: 'SEO Title', icon: Pin, content: product.seo_title || '' },
    { key: 'meta_description', label: 'Meta Description', icon: FileText, content: product.meta_description || '' },
    { key: 'long_description', label: 'Long Description', icon: BookOpen, content: product.long_description || '' },
    { key: 'benefits', label: 'Benefits', icon: Target, content: product.benefits || [] },
    { key: 'specifications', label: 'Specifications', icon: Settings, content: product.specifications || [] },
    { key: 'usage_tips', label: 'Usage Tips', icon: Lightbulb, content: product.usage_tips || [] },
    { key: 'seo_tags', label: 'SEO Keywords', icon: Tag, content: product.seo_tags || [] },
  ];
  ...
  const copyFullSheet = async () => {
    ...
    sections.forEach((s) => {
      ...
      if (Array.isArray(s.content)) {
        s.content.forEach((c) => lines.push(`- ${c}`));
      } else {
    ...
    show('Full product sheet copied', 'success');
  };

  const exportPdf = () => {
    window.print();
    show('Opening print dialog for PDF export', 'info');
  };
      ...
      >
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <h2 className="text-xl font-bold text-ink dark:text-ink-dark">{product.name}</h2>
            {product.price && (
          ...
          <div className="flex flex-wrap gap-2">
            <Button variant="outline" size="sm" onClick={copyFullSheet}>
